...
static float g_triangleShiftX = 0.0;         // move X factor for object
static float g_triangleShiftY = 0.0;         // move Y factor for object
...
static double  g_rightClickX, g_rightClickY; // coordinates for mouse right click event
...
struct SquareShaderState {
    ...
    GLint h_uWindowWidth;
    GLint h_uWindowHeight;
    GLint h_uBlender;
...
static void drawSquare() {
    ...
    safe_glUniform1i(g_squareShaderState->h_uWindowWidth, g_windowWidth);
    safe_glUniform1i(g_squareShaderState->h_uWindowHeight, g_windowHeight);
...
static void drawTriangle() {
    ...
    safe_glUniform1i(g_triangleShaderState->h_uWindowWidth, g_windowWidth);
    safe_glUniform1i(g_triangleShaderState->h_uWindowHeight, g_windowHeight);
    ...
    safe_glVertexAttribPointer(g_triangleShaderState->h_aPosition, 2, GL_FLOAT, GL_FALSE, 0, 0);

    glBindBuffer(GL_ARRAY_BUFFER, g_triangle->texVbo);
    safe_glVertexAttribPointer(g_triangleShaderState->h_aTexCoord, 2, GL_FLOAT, GL_FALSE, 0, 0);
...
static void keyboard(GLFWwindow *window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        case GLFW_KEY_S:
            ...
            break;
        case GLFW_KEY_I:
            g_triangleShiftY += 0.125;
            break;
        case GLFW_KEY_K:
            g_triangleShiftY -= 0.125;
            break;
        case GLFW_KEY_J:
            g_triangleShiftX -= 0.125;
            break;
        case GLFW_KEY_L:
            g_triangleShiftX += 0.125;
            break;
...
static void loadSquareShader(SquareShaderState &ss) {
    ...
    ss.h_uWindowWidth = safe_glGetUniformLocation(h, "uWindowWidth");
    ss.h_uWindowHeight = safe_glGetUniformLocation(h, "uWindowHeight");
...
static void loadTriangleShader(TriangleShaderState& ss) {
    const GLuint h2 = ss.program; // short hand
    ...
    // Retrieve handles to uniform variables
    ss.h_uTriangleShiftX = safe_glGetUniformLocation(h2, "uTriangleShiftX");
    ss.h_uTriangleShiftY = safe_glGetUniformLocation(h2, "uTriangleShiftY");
    ss.h_uWindowWidth = safe_glGetUniformLocation(h2, "uWindowWidth");
    ss.h_uWindowHeight = safe_glGetUniformLocation(h2, "uWindowHeight");
    ss.h_uTex2 = safe_glGetUniformLocation(h2, "uTex2");

    // Retrieve handles to vertex attributes
    ss.h_aPosition = safe_glGetAttribLocation(h2, "aPosition");
    ss.h_aTexCoord = safe_glGetAttribLocation(h2, "aTexCoord");

    glBindFragDataLocation(h2, 0, "fragColor");
    checkGlErrors();
}
...
static void loadTriangleGeometry(const GeometryPX& g) {
    GLfloat pos[9] = { 
        ...
        -0.5,  0.5,  0.5}; // I'm actually not sure what this is then. I'm guessing Z values, but why are they all at the end? Not sure.

    GLfloat tex[9] = { 
        -0.25,  1,   // top left
         1.25,  1,   // top right
         0.5, -0.25, // bottom middle
         0, 0, 0};

    glBindBuffer(GL_ARRAY_BUFFER, g.posVbo);
    glBufferData(GL_ARRAY_BUFFER, 9 * sizeof(GLfloat), pos, GL_STATIC_DRAW);
    checkGlErrors();

    glBindBuffer(GL_ARRAY_BUFFER, g.texVbo);
    glBufferData(GL_ARRAY_BUFFER, 9 * sizeof(GLfloat), tex, GL_STATIC_DRAW);
    checkGlErrors();
}
...
static void loadTexture(GLuint texHandle, const char *ppmFilename) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0,  GL_SRGB, texWidth, texHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, &pixData[0]);
